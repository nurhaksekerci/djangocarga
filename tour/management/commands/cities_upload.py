...
class Command(BaseCommand):
    help = 'Türkiye\'deki 81 ili veritabanına kaydeder'

    def handle(self, *args, **options):
        # Türkiye'deki 81 il ve plaka kodları
        cities = [
            {'name': 'Adana', 'code': '01'},
            ...
            {'name': 'Düzce', 'code': '81'},
            {'name': 'İstanbul Anadolu', 'code': '82'},
            {'name': 'İstanbul Avrupa', 'code': '83'},
        ]

        # Mevcut şehirleri temizle
        self.stdout.write('Mevcut şehirler temizleniyor...')
        City.objects.all().delete()

        # Şehirleri ekle
        self.stdout.write('Şehirler ekleniyor...')
        for city_data in cities:
            City.objects.create(**city_data)
            self.stdout.write(f"{city_data['name']} eklendi.")

        self.stdout.write(self.style.SUCCESS('Tüm şehirler başarıyla eklendi!'))
