...
from tour.models import Transfer, City

class Command(BaseCommand):
    help = 'transfer.json dosyasındaki transfer verilerini veritabanına kaydeder'
    ...
    def handle(self, *args, **options):
        ...
        json_file_path = os.path.join('data', 'transfer.json')
        ...
        try:
            ...
            # Mevcut transferleri temizle
            self.stdout.write('Mevcut transferler temizleniyor...')
            Transfer.objects.all().delete()
            
            # Transferleri ekle
            self.stdout.write('Transferler ekleniyor...')
            added_count = 0
            skipped_count = 0
            
            for transfer_data in transfers:
                # Başlangıç ve bitiş şehirlerini kontrol et
                if transfer_data['start_city'] is None or transfer_data['finish_city'] is None:
                    self.stdout.write(self.style.WARNING(f"Uyarı: {transfer_data['route']} transferi için başlangıç veya bitiş şehri belirtilmemiş. Atlanıyor."))
                    skipped_count += 1
                    continue
                
                # Şehirleri bul
                start_city = self.find_city(transfer_data['start_city'])
                end_city = self.find_city(transfer_data['finish_city'])
                
                if start_city is None or end_city is None:
                    self.stdout.write(self.style.ERROR(f"Hata: {transfer_data['route']} transferi için şehir bulunamadı: Başlangıç={transfer_data['start_city']}, Bitiş={transfer_data['finish_city']}"))
                    skipped_count += 1
                    continue
                
                # Transferi oluştur
                Transfer.objects.create(
                    name=transfer_data['route'],
                    start_city=start_city,
                    end_city=end_city
                )
                
                self.stdout.write(f"{transfer_data['route']} ({start_city} - {end_city}) eklendi.")
                added_count += 1
            
            self.stdout.write(self.style.SUCCESS(f'Tüm transferler işlendi! {added_count} transfer eklendi, {skipped_count} transfer atlandı.'))
