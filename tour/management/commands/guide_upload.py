...
from tour.models import Guide, City

class Command(BaseCommand):
    help = 'guide.json dosyasındaki rehber verilerini veritabanına kaydeder'
    ...
    def handle(self, *args, **options):
        ...
        json_file_path = os.path.join('data', 'guide.json')
        ...
        try:
            ...
            self.stdout.write('Mevcut rehberler temizleniyor...')
            Guide.objects.all().delete()
            
            # Rehberleri ekle
            self.stdout.write('Rehberler ekleniyor...')
            added_count = 0
            skipped_count = 0
            
            for guide_data in guides:
                # Şehri bul
                city = self.find_city(guide_data['new_city'])
                
                if city is None:
                    self.stdout.write(self.style.ERROR(f"Hata: {guide_data['name']} rehberi için şehir bulunamadı: {guide_data['new_city']}"))
                    skipped_count += 1
                    continue
                
                # Telefon numarasını normalize et
                phone = self.normalize_phone(guide_data['phone'])
                
                # Rehberi oluştur
                guide = Guide.objects.create(
                    name=guide_data['name'],
                    phone=phone,
                    document_no=guide_data['doc_no'] if guide_data['doc_no'] else ""
                )
                
                # Şehri ekle
                guide.cities.add(city)
                
                self.stdout.write(f"{guide_data['name']} ({city}) eklendi.")
                added_count += 1
            
            self.stdout.write(self.style.SUCCESS(f'Tüm rehberler işlendi! {added_count} rehber eklendi, {skipped_count} rehber atlandı.'))
